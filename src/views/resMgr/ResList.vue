...
<template>
  <section>
      ...
      <el-col :span="24" class="toolbar">
        <el-form :inline="true" v-show="!noTool" @submit.native.prevent>
          ...
          <el-form-item label="模糊查询：">
            <el-input size="small" v-model="queryParams.queryKey" placeholder="模糊查询" @input="debounceQuery"></el-input>
          </el-form-item>
          <el-form-item  v-if="!noQuery" label="精确查询：">
            <el-select style="width: 100%" @change="queryColumnChange" v-model="listQuery" value-key="COLUMN_NAME">
              <el-option v-for="(item, key) in queryRows" :key="key" :label="item.COLUMN_CNAME"
                :value="item"  ></el-option>
            </el-select>
          </el-form-item>
          <el-form-item v-if="!noQuery">
            <el-select  v-if="listQuery && listQuery.PROPERTY_TYPE === '2'" style="width: 100%" filterable v-model="listQuery.value" value-key="COLUMN_NAME">
              <el-option v-for="(item, key) in filtersList" :key="key" :label="item.text"
                :value="item.value"  ></el-option>
            </el-select>
            <el-date-picker v-else-if="listQuery.PROPERTY_TYPE === '5'" v-model="listQuery.value" value-format="yyyy-MM-dd" type="daterange" placeholder="选择日期" unlink-panels
      range-separator="至"  start-placeholder="开始日期"  end-placeholder="结束日期"></el-date-picker>
            <el-input v-else size="small" v-model="listQuery.value" placeholder="字段查询"></el-input></el-form-item>
          <el-form-item>
    ...
    <!--列表-->
    <el-table ref="elTable" :span-method="objectSpanMethod" highlight-current-row @sort-change="tableSort"
      @selection-change="selsChange" @row-click="rowClick" :data="resDatas" :row-class-name="tableRowClassName"
      v-loading="listLoading" header-cell-class-name="table_th" border height="70vh" style="width: 100%">
      ...
      <el-table-column :sortable="row.IS_SORT == '1' ? 'custom' : false" align="center" v-for="(row, index) in listRows"
        :key="index" :prop="row.COLUMN_NAME" :fixed="row.IS_FROZEN == 1 ? 'left' : false" :label="row.COLUMN_CNAME"
    ...
    <el-col :span="24" class="pagination">
      ...
      <el-pagination background @current-change="handleCurrentChange" :current-page="queryParams.pageNumber"
        :page-sizes="[30, 60, 100, 150]" :page-size="queryParams.pageSize"
        layout="total, sizes, prev, pager, next, jumper" :total="total" @size-change="sizeChange"></el-pagination>
    </el-col>
...
export default {
  ...
  computed: {
    ...
    listRows() {
      return this.resRows.filter((item) => {
        return item.PROPERTY_TYPE !== "10";
      });
    },
    queryRows() {
      return this.resRows.filter((item) => {
  ...
  methods: {
    ...
    queryResList() {
      ...
      if (this.listQuery.COLUMN_NAME) {
        this.queryParams.query = {}
        this.queryParams.query[this.listQuery.COLUMN_NAME] = this.listQuery.value;
      }
    ...
    handleExport() {
      ...
      this.$ajax
        .getBolb(this.$api.exportExcel, this.queryParams)
        .then((res) => {
          if (res.data) {
