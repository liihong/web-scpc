<template>
  <div class="blkcgl">
    <ResList tableId="010404" @selectChange="selectChange" :query="this.$route.query" noEdit noAdd ref="blList">
       <el-form-item slot="toolBar">
        <el-button type="primary" icon="el-icon-s-unfold" @click="manyBOMBl" class="radio" :label="1">批量通过</el-button>
      </el-form-item>
      <template slot="CLZT" slot-scope="scope">
        <el-radio-group v-model="scope.row.CLZT" class="radioGroup">
          ...
          <el-radio
            v-show="scope.row.CLZT !=0 && scope.row.CLZT !=2"
        ...
        <span
          @click="cgClick(scope.row)"
          v-show="scope.row.CLZT != '' || scope.row.CLZT != null"
          class="spanText"
        >{{clztData[scope.row.CLZT]}}</span>
      </template>
      <template slot="SSDD" slot-scope="scope">
        <el-badge
          v-show="!!scope.row.ZDDJB"
          :value="scope.row.ZDDJB == '0402' ? '重要' : scope.row.ZDDJB == '0403' ? '' : '紧急'"
          class="item"
          :type="scope.row.ZDDJB == '0402' ? 'warning' : scope.row.ZDDJB == '0403' ? 'info' : 'danger'"
        >
