  ...
  <el-dialog title="工艺编排" :visible.sync="dialogState.show" width="80%">
    ...
    <el-row>
      <el-col v-show="type == 'edit'" :span="22">
        <draggable class="list-group" v-model="jggyList"
          :options="{draggable:'.el-tag',filter: '.undraggable', sort: false}" @end="drop">
          <el-tag class="elTag" v-for="(item,index) in jggyList" :value="item.id" :name="item.gymc" :key="index">
            {{item.gymc}}</el-tag>
        </draggable>
      </el-col>
        ...
        <el-table :data="gygxList" class="gygxTable" id="gygxTable" row-key="id">
          <el-table-column min-width="20" align="center" v-if="type == 'edit'" property="gxnr" label="删除">
            <template slot-scope="scope">
              <el-button @click="deleteGy(scope.$index,scope.row)" size="mini" type="danger" icon="el-icon-delete"
                circle></el-button>
            </template>
          </el-table-column>
          <el-table-column width="30" type="index" align="center"></el-table-column>
          <el-table-column width="40" align="center" property="sfwx" label="外协">
            <template slot-scope="scope">
              <el-switch v-model="scope.row['sfwx']" :active-value=1 :inactive-value=0>
              </el-switch>
            </template>
          </el-table-column>
          <el-table-column class-name="gxnr" align="center" property="gxnr" label="工序" min-width="50">
            <template slot-scope="scope">
              <el-select @change="((gyid)=>{changeGygx(gyid,scope.row)})" size="mini" v-model="scope.row['gynr']"
                placeholder="请选择">
          ...
          <el-table-column property="edgs" label="额定工时(分钟)" min-width="70">
            <template slot-scope="scope">
              <el-input-number v-if="type == 'edit'" :controls=false size="mini" v-model="scope.row['edgs']" :min="0"
          ...
          <el-table-column property="bzgs" label="总工时(分钟)" min-width="70">
            <template slot-scope="scope">
              <el-input-number v-if="type == 'edit'" :controls=false size="mini" v-model="scope.row['bzgs']" :min="0"
          ...
          <el-table-column property="zbgs" label="准备工时" min-width="70">
            <template slot-scope="scope">
              <el-input-number v-if="type == 'edit'" :controls=false size="mini" v-model="scope.row['zbgs']" :min="0"
          ...
          <el-table-column property="zysx" label="工艺内容" min-width="250">
            <template slot-scope="scope">
              <el-input v-if="type == 'edit'" size="mini" type="textarea" :rows="2" v-model="scope.row['zysx']"
