    ...
    >
      ...
      >
        <template slot-scope="scope">
          <el-button
            v-if="!scope.row.CZRYID"
            size="mini"
            type="primary"
            @click="beginWork(scope.row)"
            >开始</el-button
          >
          <el-button
            v-if="scope.row.CZRYID"
      ...
      <template slot="SSDD" slot-scope="scope">
        <el-badge
          v-show="!!scope.row.DDLEVEL"
          :value="
            scope.row.DDLEVEL == '0402'
              ? '重要'
              : scope.row.DDLEVEL == '0403'
              ? ''
              : '紧急'
          "
          class="item"
      ...
      <template slot="BOMID" slot-scope="scope">
        <span class="spanText" @click="gybpClick(scope.row)">{{
          scope.row.BOMID_TEXT
        }}</span>
      </template>
      <template slot="GYNR" slot-scope="scope">
        <a v-if="!!scope.row.DDTZ" :href="scope.row.DDTZ" target="_blank">
          <span :style="{ color: scope.row.CZRYID ? 'red' : '' }">
            {{ scope.row.GYNR_TEXT }}</span
          >
        </a>
        <span v-else :style="{ color: scope.row.CZRYID ? 'red' : '' }">
          {{ scope.row.GYNR_TEXT }}</span
        >
...
export default {
  ...
  computed: {
    ...mapGetters(["fzgy", "wxSelRow"]),
  },
  created() {
    if (sessionStorage.getItem("wxSelRow")) {
      sessionStorage.removeItem("wxSelRow");
    }
  },
  methods: {
    getChecks(sel) {
    ...
    getResList() {
      let selRow = JSON.parse(sessionStorage.getItem("wxSelRow"));
      if (selRow) {
        if (this.selectRows) {
          selRow = [...selRow, ...this.selectRows];
          sessionStorage.setItem("wxSelRow", JSON.stringify(selRow));
        }
      } else {
        if (this.selectRows) {
          sessionStorage.setItem("wxSelRow", JSON.stringify(this.selectRows));
        }
      }
    },
    //显示工艺编排信息
    ...
    passMany() {
      if (this.selectRows && this.selectRows.length > 0) {
        ...
        if(selRow){
          selRow = [...selRow, ...this.selectRows];
          sessionStorage.setItem("wxSelRow", JSON.stringify(selRow));
          this.passManyState.selectRows = selRow;
        } else {
