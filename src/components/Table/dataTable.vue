...
<template>
  <section>
    ...
    <el-table class="el-table" :data="tableData" v-loading="listLoading" stripe border :max-height="tableHeight" style="width: 100%;">
      <el-table-column v-for="(row,index) in columnDatas" :key="index" :prop="row.id" align="center" :fixed="(row.frozen == 1?'left':false)" :label="row.name" :min-width="(row.length != '')?row.length:150">
...
export default {
  name: 'dataTable',
  extends: Table,
  props: ['priarmyKey','columnDatas', 'tableDatas','tableInfo','nohandle', 'showDetail'],
  data() {
    return {
      filters: {
        columns: '',
